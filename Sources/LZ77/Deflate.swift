// Deflate.swift - compressing to a zlib stream without a C library underneath
//
// The counterpart to Inflate, and the same push-in/pull-out shape: rows go in as the encoder
// above produces them, compressed bytes come out whenever there is room, and neither side
// dictates the other's block size.
//
...
public final class Deflate {
    ...
    private func writeZlibHeader() {
        // CMF: deflate with a 32 KiB window. FLG: no preset dictionary, and a check byte chosen
        // so the pair is a multiple of 31, which is what RFC 1950 §2.2 requires.
        let cmf: UInt32 = 0x78
        var flg: UInt32 = 0
        ...
        self.writer.write(cmf, bits: 8)
        self.writer.write(flg, bits: 8)
    }
    ...
    private func finishStream() {
        ...
        self.writer.append(UInt8(truncatingIfNeeded: value >> 24))
        self.writer.append(UInt8(truncatingIfNeeded: value >> 16))
        self.writer.append(UInt8(truncatingIfNeeded: value >> 8))
        self.writer.append(UInt8(truncatingIfNeeded: value))
    ...
    /// Drops history no match can reach any more, so `pending` does not grow with the image.
