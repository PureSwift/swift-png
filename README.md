...
colour-mapped output for anything but a plain opaque greyscale or RGB file, discarding alpha onto a
buffer with no background named, and discarding colour from a file that also carries coverage. Each
of those refuses outright and says why, rather than producing an answer that is nearly right; the
