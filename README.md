...
colour-mapped output for anything with real coverage of its own to remove or colour reduced to
greyscale (or the reverse), discarding alpha onto a buffer with no background named, and discarding
colour from a file that also carries coverage. Each of those refuses outright and says why, rather
than producing an answer that is nearly right; the general read and write API these shortcuts sit
in front of has no such gap, and the writing side of the shortcuts no longer has one either.
